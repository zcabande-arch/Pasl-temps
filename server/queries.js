...
module.exports = {
  ...
  pushSet: `INSERT INTO push_subs (endpoint, uid, sub, hour, tz, every, next_at) VALUES (?, ?, ?, ?, ?, ?, ?)
    ON CONFLICT(endpoint) DO UPDATE SET uid = excluded.uid, sub = excluded.sub, hour = excluded.hour, tz = excluded.tz, every = excluded.every, next_at = excluded.next_at`,
  pushDel: "DELETE FROM push_subs WHERE endpoint = ?",
  pushDelUid: "DELETE FROM push_subs WHERE uid = ?",
  pushDue: "SELECT endpoint, sub, hour, tz, every, next_at FROM push_subs WHERE next_at <= ? ORDER BY next_at LIMIT ?",
